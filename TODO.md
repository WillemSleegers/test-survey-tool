...
- [x] Remove render-time mutation of shared state — **done**: `computedVar.value` deleted from `ComputedVariable` and the writes in `computed-variables.ts` removed (no readers existed outside the module); breakdown subtotal computation extracted into `computeSubtotalValues`/`subtotalVariables` (`lib/breakdown-calculations.ts`) as a single order-independent pass, shared by `breakdown-question.tsx` and `lib/response-variables.ts` (which had the identical duplicated bug). Tests in `lib/breakdown-calculations.test.ts`, `lib/response-variables.test.ts`, `lib/conditions/computed-variables.test.ts`
